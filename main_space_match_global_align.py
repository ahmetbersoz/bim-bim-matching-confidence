...
def apply_transform_to_indices(comps: List[Comp], indices: List[int], T: np.ndarray) -> None:
    ...
    for idx in indices:
        ...
        comp.obb = _obb_from_mesh(comp.mesh)
...
def save_space_csvs(prefix: str, gt_space: Comp, metrics: Dict[str, Any]) -> None:
    ...
    space_prefix = f"{prefix}_{label}_{gt_space.idx}"
    _write_csv_bundle(
        space_prefix,
        metrics.get("per_gt", []),
...
def write_floor_area_csv(path: str, records: List[Dict[str, Any]]) -> None:
    os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow([
            "gt_guid", "gt_name", "pred_guid", "pred_name",
            "space_iou_pre", "space_iou_post",
        ...
        for rec in records:
            w.writerow([
                ...
                rec.get("pred_guid"),
                rec.get("pred_name"),
                f'{float(rec.get("space_iou_pre", 0.0)):.6f}',
                f'{float(rec.get("space_iou_post", 0.0)):.6f}',
...
def _obb_to_lineset(obb: OBB, color=(1.0, 0.0, 0.0)) -> o3d.geometry.LineSet:
    ...
    # edges of a box (pairs of indices into corners)
    edges = [
        [0, 1], [1, 2], [2, 3], [3, 0],
        [4, 5], [5, 6], [6, 7], [7, 4],
        [0, 4], [1, 5], [2, 6], [3, 7]
...
def main():
    ...
    ap.add_argument("--target-space-guid", required=True, help="GlobalId of the GT IfcSpace used to derive the ICP refinement.")
    ...
    ap.add_argument("--space-align", choices=["icp", "icp_xy", "centroid", "none"], default="none",
    ...
    args.target_space_guid = (args.target_space_guid or "").strip()
    if not args.target_space_guid:
        raise ValueError("Argument --target-space-guid must be a non-empty IfcSpace GlobalId.")

    # Enforce the new workflow: centroid global alignment followed by targeted ICP refinement.
    if args.align.lower() != "centroid":
        log_step("Overriding --align to 'centroid' for the required workflow.")
        args.align = "centroid"
    if args.space_align.lower() != "none":
        log_step("Overriding --space-align to 'none'; targeted ICP refinement is handled separately.")
    ...
    target_guid_upper = args.target_space_guid.upper()
    ...
    for match_idx, (i_gt, j_pr, score) in enumerate(matches):
        ...
        pre_iou = float(score)
        match_label = "(target)" if is_target_space else ""

        matched_gt_space_indices.add(i_gt)
        matched_pr_space_indices.add(j_pr)

        log_step(f"Aligning matched space pair {match_idx + 1}/{len(matches)} {match_label} (IoU_pre={pre_iou:.3f})")
        # Print GT and PRED space OBBs before per-space alignment
        _print_spaces_summary(f"GT space (match {match_idx+1})", [gt_space], max_print=1)
        _print_spaces_summary(f"PRED space (match {match_idx+1}, before)", [pr_space], max_print=1)

        if is_target_space:
            T_space = np.eye(4, dtype=float)
            recorded_alignment_matrix = target_icp_alignment
            alignment_mode = "target_icp"
        else:
            T_space = align_space_pair(gt_space, pr_space, args.space_align)
            recorded_alignment_matrix = T_space
            alignment_mode = args.space_align
        apply_transform_to_indices(spaces_pr, [j_pr], T_space)

        # Print PRED space after per-space alignment
        _print_spaces_summary(f"PRED space (match {match_idx+1}, after)", [spaces_pr[j_pr]], max_print=1)

        if args.visualize_per_space:
            try:
                _visualize_spaces([gt_space], [spaces_pr[j_pr]], title=f"Match {match_idx+1}: GT vs PRED (after)")
            except Exception as exc:
        ...
        floor_area_records.append({
            ...
            "space_iou_pre": pre_iou,
            "space_iou_post": float(post_iou),
        ...
        alignment_info = {
            "mode": alignment_mode,
            "T": _matrix_to_nested_list(recorded_alignment_matrix),
            "space_iou_pre": float(pre_iou),
            "space_iou_post": float(post_iou),
            "is_target": is_target_space,
            "transformed_pred_element_indices": transform_candidates,
        }
        if skipped:
        ...
        by_space[by_space_key] = {
            ...
            "pred_space": {"guid": pr_guid, "name": pr_space.meta.Name, "meta": asdict(pr_space.meta)},
    ...
    gt_elements_without_space = [idx for idx, comp in enumerate(elems_gt) if not elem_guid_to_space_gt.get(comp.guid)]
    pred_elements_without_space = [idx for idx, comp in enumerate(elems_pr) if not elem_guid_to_space_pr.get(comp.guid)]
    ...
    unmatched_bucket = None
    if args.include_unmatched == "global" and gt_unmatched_elem_indices and pred_unmatched_elem_indices:
        ...
        if args.align != "none":
            log_step("Aligning unmatched bucket")
            if args.align == "icp":
        ...
        transformed_pred_elem_indices.update(unmatched_pr_indices_sorted)
    ...
    if args.include_unmatched == "global":
    ...
    unmatched_summary = {
        ...
        "gt_spaces_ignored": len(unmatched_gt_space_indices) if args.include_unmatched == "ignore" else 0,
        "pred_spaces_ignored": len(unmatched_pr_space_indices) if args.include_unmatched == "ignore" else 0,
        "gt_elements_ignored": len(gt_unmatched_elem_indices) if args.include_unmatched == "ignore" else 0,
        "pred_elements_ignored": len(pred_unmatched_elem_indices) if args.include_unmatched == "ignore" else 0,
        "gt_elements_in_unmatched_bucket": len(gt_unmatched_elem_indices) if args.include_unmatched == "global" else 0,
        "pred_elements_in_unmatched_bucket": len(pred_unmatched_elem_indices) if args.include_unmatched == "global" else 0,
        "gt_elements_without_space": len(gt_elements_without_space),
        "pred_elements_without_space": len(pred_elements_without_space)
    }

    mesh_output_dir_abs = os.path.abspath(args.mesh_output_dir)
    ...
    floor_area_csv_abs = os.path.abspath(args.floor_area_csv)
    ...
    report_config = {
        ...
        "floor_area_csv": floor_area_csv_abs
    }

    report: Dict[str, Any] = {
        "config": report_config,
        "global_alignment": {"mode": global_alignment_mode, "T": _matrix_to_nested_list(global_alignment)},
        "overall": global_metrics["overall"],
        "per_gt": global_metrics["per_gt"],
        "per_pred": global_metrics["per_pred"],
        "correspondences": global_metrics["correspondences"],
        "by_space": by_space,
        "space_matches": space_match_records,
        "unmatched_summary": unmatched_summary,
        "target_refinement": target_refinement_info,
        "floor_area_records": floor_area_records,
        "floor_area_csv_path": floor_area_csv_abs,
        "mesh_exports": {
    ...
    if unmatched_bucket is not None:
        report["unmatched_bucket"] = unmatched_bucket
    ...
    write_floor_area_csv(floor_area_csv_abs, floor_area_records)
    ...
    if matches:
        ...
        for rec in space_match_records[:5]:
            pre = float(rec.get("space_iou_pre", 0.0))
            post = float(rec.get("space_iou_post", pre))
            flag = " (target)" if rec.get("is_target") else ""
            print(f"  GT {rec['gt_guid']} -> PRED {rec['pred_guid']} | IoU_pre={pre:.3f} -> IoU_post={post:.3f}{flag}")
    else:
        print("  No matched spaces.")

    print("\n=== Overall (aggregated elements) ===")
    for k, v in global_metrics["overall"].items():
        print(f"{k}: {v:.6f}")

    print("\nUnmatched summary:")
    print(f"  Mode: {args.include_unmatched}")
    print(f"  GT spaces unmatched: {unmatched_summary['gt_spaces_unmatched']} (ignored: {unmatched_summary['gt_spaces_ignored']})")
    print(f"  PRED spaces unmatched: {unmatched_summary['pred_spaces_unmatched']} (ignored: {unmatched_summary['pred_spaces_ignored']})")
    print(f"  GT elements unmatched: {unmatched_summary['gt_elements_unmatched']} (ignored: {unmatched_summary['gt_elements_ignored']})")
    print(f"  PRED elements unmatched: {unmatched_summary['pred_elements_unmatched']} (ignored: {unmatched_summary['pred_elements_ignored']})")
    ...
    if args.save_csv_prefix:
        save_csvs(args.save_csv_prefix, report)
        log_step(f"Saved CSVs with prefix: {args.save_csv_prefix}")
        for gt_space, metrics in per_space_csv_payload:
            save_space_csvs(args.save_csv_prefix, gt_space, metrics)
        if per_space_csv_payload:
            log_step(f"Saved per-space CSV bundles ({len(per_space_csv_payload)} spaces)")
        if unmatched_bucket is not None:
            unmatched_prefix = f"{args.save_csv_prefix}_unmatched"
            _write_csv_bundle(
                unmatched_prefix,
            ...
            log_step("Saved unmatched bucket CSV bundle")
