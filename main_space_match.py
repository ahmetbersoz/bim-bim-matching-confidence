...
def main():
    ...
    for match_idx, (i_gt, j_pr, score) in enumerate(matches):
        ...
        log_step(f"Aligning matched space pair {match_idx + 1}/{len(matches)} (IoU={score:.3f})")
        ...
        alignment_info = {
            "mode": args.space_align,
            "T": _matrix_to_nested_list(T_space),
            "space_iou": float(score),
    ...
    report: Dict[str, Any] = {
        ...
        "global_alignment": {"mode": args.align, "T": _matrix_to_nested_list(global_alignment)},
    ...
    if matches:
        ...
        for rec in space_match_records[:5]:
            print(f"  GT {rec['gt_guid']} -> PRED {rec['pred_guid']} | IoU={rec['space_iou']:.3f}")
    else:
